...
import { Phone, MessageCircle, MessageSquare, LogOut, Search, Loader2, MapPin, Ban, Calendar, IndianRupee } from "lucide-react";
...
const TelecallerDashboard = () => {
  ...
  const load = async () => {
    setLoading(true);
    // Today's calling list: call_date <= today, status NOT Unsubscribed/Done
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((lead) => {
              ...
              return (
                <Card key={lead.id} className={`overflow-hidden transition-shadow hover:shadow-elegant ${overdue ? "border-destructive/40" : ""}`}>
