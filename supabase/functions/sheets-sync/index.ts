...
// Imports leads from a publicly-published Google Sheet (CSV export).
// User must publish the sheet: File > Share > Publish to web > CSV.
// Expected headers: customer_name, phone_number, area_id (or area_name), policy_type, registration_number, premium_amount
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!sheet_url) return new Response(JSON.stringify({ error: "sheet_url required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
